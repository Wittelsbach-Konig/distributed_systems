...
void set_pipe_descriptors(process_content* processContent, int process_count) {
    for(uint8_t from = 0; from < process_count - 1; ++from){
        for(uint8_t to = 1 + from; to < process_count; ++to){
            ...
            logging_pipes_created(from, to, file_descriptors[0], file_descriptors[1]);  // logger
            ...
            // add logging
            logging_pipes_created(to, from, file_descriptors1[0], file_descriptors1[1]);  // logger
        }
    }
}

void set_nonblocking_flags(int file_descriptor){
    int flags = fcntl(file_descriptor, F_GETFL, 0);
    fcntl(file_descriptor, F_SETFL, flags | O_NONBLOCK);
}

void modify_pipe_set_non_blocking(process_content* processContent, int process_count){
    for(uint8_t from = 0; from < process_count - 1; ++from){
        for(uint8_t to = 1 + from; to < process_count; ++to){
            set_nonblocking_flags(processContent->read_pipes[from][to]);
            set_nonblocking_flags(processContent->write_pipes[from][to]);
            set_nonblocking_flags(processContent->read_pipes[to][from]);
            set_nonblocking_flags(processContent->write_pipes[to][from]);
        }
    }
}
